...
export const ShiftNotificationItem: React.FC<ShiftNotificationItemProps> = ({
  ...
  // Function to translate shift notification content
  const getTranslatedTitle = () => {
    const type = notification.type?.toLowerCase();
    if (type?.includes('shift') || type?.includes('směna')) {
      if (notification.title?.includes('Zítra') || notification.title?.includes('Tomorrow')) {
        return t('shift.newShift') + ' - ' + t('time.tomorrow', 'Zítra');
      }
      if (notification.title?.includes('směna') || notification.title?.includes('Shift')) {
        return t('shift.newShift');
      }
    }
  ...
  const getTranslatedMessage = () => {
    ...
    if (type?.includes('shift') || type?.includes('směna')) {
      ...
      const timeLabel = startTime && endTime ? `${formatTime(startTime)} - ${formatTime(endTime)}` : '';
      return `${shiftTypeLabel} ${timeLabel}`;
    }
