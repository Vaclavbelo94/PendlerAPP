...
import { Download, Upload, AlertCircle, FileJson, Files, FileText } from 'lucide-react';
...
const VocabularyImportExport: React.FC<VocabularyImportExportProps> = ({ vocabularyItems, onImport }) => {
  ...
  const [activeTab, setActiveTab] = useState<'json' | 'csv' | 'txt'>('json');
  ...
  // Export vocabulary items as TXT (simple format)
  const handleExportTXT = () => {
    try {
      // Simple text format: word - translation
      const txtContent = vocabularyItems
        .map(item => `${item.word} - ${item.translation}${item.example ? ` - ${item.example}` : ''}`)
        .join('\n');
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `german-vocabulary-${new Date().toISOString().split('T')[0]}.txt`;
      document.body.appendChild(a);
      a.click();
      ...
      toast({
        title: "Export úspěšný",
        description: `Exportováno ${vocabularyItems.length} slovíček ve formátu TXT.`,
      });
    } catch (error) {
      console.error('Export failed:', error);
      toast({
  ...
  const handleImportTXT = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        lines.forEach(line => {
          // Parse line in format: "word - translation - example"
          ...
          const example = parts.length > 2 ? parts.slice(2).join(' - ').trim() : undefined;
        ...
        toast({
          title: "Import úspěšný",
          description: `Importováno ${items.length} slovíček ze souboru TXT.`,
        });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Zálohujte svá slovíčka nebo importujte nová{isOffline ? " (nyní offline)" : ""}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'json' | 'csv' | 'txt')}>
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="txt" className="flex items-center">
          ...
          <TabsContent value="txt" className="space-y-4 pt-2">
            <div className="flex flex-col sm:flex-row gap-2">
              <Button 
                variant="outline" 
              ...
              <div className="relative">
                ...
                />
                <Button asChild>
                  <label htmlFor="txt-import" className="flex items-center cursor-pointer">
                    <Upload className="mr-2 h-4 w-4" />
                    Importovat TXT
                  </label>
                </Button>
            ...
            <div className="text-sm text-muted-foreground">
              <p>TXT formát je nejjednodušší varianta - každý řádek ve formátu "slovo - překlad - příklad".</p>
            </div>
          </TabsContent>
        </Tabs>
        ...
        {isOffline && (
          <Alert>
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              Jste v offline režimu. Při exportu budou uložena pouze lokálně dostupná data.
            </AlertDescription>
          </Alert>
        )}
      </CardContent>
